...
def gauss_function(theoretical_center, moltiplicative_factor, empirical_center, sigma):
    """
    funzione gaussiana
    x : media teorica
    ...
    sigma : varianza
    """
    return moltiplicative_factor * np.exp(- (theoretical_center - empirical_center)**2/(2 * sigma**2))
...
for i in range(0, 5):
    ...
    if An > t_s * dAn:  # se supera e'
        ...
        # ampiezza
        a0, da, x0, dx0, sigma, dsigma = fit_gauss(x_peak, yp, dy_peak)
        X0 = np.append(X0, x0)
