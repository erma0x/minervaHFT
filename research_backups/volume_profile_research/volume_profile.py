...
def calc_volume_profile(df, period, bins):
    ...
    data = []
    # assume this is volume center per each 1m candle
...
def calc_vwap(period):
    ...
    for _, g in df.groupby(pd.Grouper(key='time', freq=period)):
        ...
        vwap = vwap.append(
            g.hlc3v.loc[i0:i1].cumsum() / df.volume.loc[i0:i1].cumsum())
    return vwap
...
time_volume_profile = calc_volume_profile(df, period='W', bins=20)
# try period='D'     # calc_volume_profile period W or d
vwap = calc_vwap(period='D')
# plot
