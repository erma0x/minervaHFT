...
HEADER_TABLE = ['open_time','high','low','close','volume','close_time','quote_asset_volume','']

def print_all(db):
    connection = sqlite3.connect(db)
    cursor = connection.cursor()
    rows = cursor.execute("SELECT id, date, sender, message, placed FROM price").fetchall()
    print(rows)
    connection.close()
...
def write_message(id_message, message_date,sender_group,text_message,NAME_DB):
    ...
    c = conn.cursor()
    query = """ INSERT INTO messages VALUES ({0},'{1}','{2}','{3}','ok')""".format(id_message ,message_date,sender_group,text_message)
    c.execute(query)
    conn.commit()
...
#create_db('pluto.db')
write_message(id_message=6, message_date='10.10',sender_group='b',text_message='test',NAME_DB='pluto.db')
print_all('pluto.db')
