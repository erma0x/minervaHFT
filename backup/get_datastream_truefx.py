...
import requests
import pandas as pd
from parameters import INTERVAL_OF_SECONDS, DATA_PROVIDER_LINK, DATA_FILE_NAME
...
def from_webpage_to_data(CONTENT):
    ...
    """
    n1 = str(CONTENT)[2:].replace("<table>","").replace("<tr>","").replace("<td>","").split("</td></tr>USD/JPY</td>")[0].split("</td>") #<table><tr><td>
    ...
    while True:
        ...
        try:
            ...
            if str(r) == "<Response [200]>":
                ...
                row = str(current_time)[:-7]+', '+ str(list(price.values())).replace('[','').replace(']','')+'\n'
                ...
                time.sleep(INTERVAL_OF_SECONDS - 1 ) # - 1 for process time and async request
