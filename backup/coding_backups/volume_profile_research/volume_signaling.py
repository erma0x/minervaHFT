...
def fit_gauss(x,y,dy,TF):
    ...
    #Calcolo parametri
    popt, pcov = curve_fit(gauss_function, x, y, p0 = [1, mean, sigma])#, sigma = dy)
    ...
    if TF:
        ...
        print("err (dev. standard su x0) = %f +- %f" % (np.sqrt(np.abs(sigma)), np.sqrt(dsigma)))

        print("Chisquare/ndof = %f/%d, (Aspettative: %f vs. %f)" % (chisq, ndof,chisq/ndof ,np.sqrt(8)))

        pylab.figure()
        pylab.errorbar(x, y, dy, linestyle = '', color = "black", marker = "o",label='Dati picco')
...
yf1 = pylab.loadtxt(sys.path[0]+"/example_volume_data.txt", skiprows=12,unpack = True,max_rows=2048)
dyf1=np.sqrt(yf1)
...
for i in range (0,5):
    ...
    pylab.figure(1)
    pylab.errorbar(x_peak,y_peak,dy_peak,linestyle = '', marker = "o", color = "red")
    ...
    if An > t_s * dAn : # se supera e'  
        ...
        y_cont= (y_peak[N-1]-y_peak[0])/(x_peak[N-1]-x_peak[0])*x_peak+ (((x_peak[N-1]*y_peak[0])-(x_peak[0]*y_peak[N-1]))/(x_peak[N-1]-x_peak[0]))
        yp=y_peak-y_cont
