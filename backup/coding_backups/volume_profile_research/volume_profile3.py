...
def load_ETHUSD_2021():
    df = pd.read_csv(
        '/home/kodachi/Documents/WhiteDragon/backup/ETHUSD_1m_2021_2_months.csv', skiprows=0)
    del df['unix']
    del df['symbol']
    del df['Volume ETH']
...
if __name__ == "__main__":
    sd = datetime(2022, 9, 10)
    ed = datetime.now()

    #df = yf.download(tickers='APPL', start=sd, end=ed, interval="1m")
    ...
    fig = market_profile(df, price_pace=0.5, return_raw=False)
    fig.show()
