...
from configuration_backtest import DATA_FILE_NAME, INTERVAL_OF_SECONDS, header
...
def market_profile(df, price_pace=0.25, time_pace='30T', return_raw=False):
    ...
    tpo_bars = np.zeros([price_buckets.shape[0] - 1,
                        df_agg.shape[0]], dtype=np.int32)
    j = 0
...
def load_data():
    df = pd.read_csv(sys.path[0] + '/BTCUSDT.csv')  # ,skiprows=0)

    del df['start_time']
    del df['end_time']
    del df['quote_asset_volume']
    del df['numbers_of_trades']
    del df['ignore']
    del df['taker_buy_base_asset_volume']
    del df['taker_sell_base_asset_volume']
    del df['maker_sell_base_asset_volume']
    del df['maker_buy_base_asset_volume']
...
def get_price(market='BTCUSDT', tick_interval='1m'):
    url = 'https://api.binance.com/api/v3/klines?symbol=' + \
        market+'&interval='+tick_interval+'&limit=1'
...
if __name__ == '__main__':
    ...
    while True:
        ...
        price = get_price(market='BTCUSDT', tick_interval='1m')
        ...
        # WRITE PRICE DATA IN CSV FILE
        row = str(price).replace('[', '').replace(
            ']', '').replace("'", "") + '\n'
