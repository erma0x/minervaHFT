...
from parameters import DATASTREAMER_URL, MARKET, TOPICS_DATASTRAMER, REQUEST_TIME_INTERVAL, LIMIT_KLINES
...
def download_price_history(symbol='BTCUSDT', start_time='2020-06-22', end_time='2020-08-19', interval_mins=1):
    ...
    for start_t in range(start_time, end_time, 1000*interval_ms):
        ...
        url = 'https://api.binance.us/api/v3/klines?interval=%s&limit=%s&symbol=%s&startTime=%s&endTime=%s' % (interval_str, LIMIT_KLINES, symbol, start_t, end_t)       
...
def datastream_publisher():
    ...
    while True:
        ...
        for topic in TOPICS_DATASTRAMER:
            ...
            if topic == 'orderbook':
                object = get_orderbook_depth(ticker = MARKET, limit_ = 100 ) # order book
            ...
            if topic == 'klines' : # and klines_required
                ...
                price_history_dataFrame = download_price_history(symbol='BTCUSDT',start_time='2022-11-01', end_time='2022-11-02')
