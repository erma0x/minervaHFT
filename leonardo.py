...
from configurations import TRADING_OPERATION, REQUEST_TIME_INTERVAL
...
if __name__ == '__main__':
    ...
    while True:
        ...
        OPERATION_SIDE = ""
        if OPERATION_PARAMETER > THESHOLD_SHORT and SHORT_OPERATIONS <= math.floor(MAX_CONCURRENT_OPEN_TRADES/2):
            OPERATION_SIDE = "SHORT" 

        elif OPERATION_PARAMETER < THESHOLD_LONG and LONG_OPERATIONS <= math.floor(MAX_CONCURRENT_OPEN_TRADES/2):
            OPERATION_SIDE = "LONG" 
        ...
        REWARD_RISK_RATEO = 0
        if peaks_bids.any():
            if len(TRADE_ORDERBOOK) < MAX_CONCURRENT_OPEN_TRADES:
        ...
        if my_timer.elapsed < REQUEST_TIME_INTERVAL:
            time.sleep( REQUEST_TIME_INTERVAL + 0.01 - my_timer.elapsed )
                                          ...
                                          # implement here
        if EQUITY < INITIAL_CAPITAL*0.03: # max drowdown!
            break
