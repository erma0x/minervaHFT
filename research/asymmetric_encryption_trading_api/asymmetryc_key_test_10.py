...
def hashing(query_string):
    return hmac.new(
        #SECRET.encode("utf-8"), query_string.encode("utf-8"), hashlib.sha256).hexdigest()
        SECRET, query_string.encode("utf-8"), hashlib.sha256).hexdigest()
...
def send_signed_request(http_method, url_path, payload={}):
    ...
    #timestamp =str(int(time.time() * 1000)- 15000)
    timestamp = send_public_request("GET", "/api/v3/time")
    ...
    params = {"url": url, "params": {'recvWindow': 60000}}
